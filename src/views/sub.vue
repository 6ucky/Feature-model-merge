...
export default {
	...
	methods: {
		...
		checkconstraints(index){
			...
			if(checkindex !== -1)
			{
				if(this.checkadditionalconstraint(xmlobject.mxGraphModel.root,this.data[checkindex].data.nodeId,modeltype))
					this.checkconstraints(checkindex);
			}
		},
		/**
		 * check the checked constraint element if it has additional constraints
		 * @param {number} id			- the id of the checked constraint element
		 * @param {string} modeltype	- the type of the diagram
		 */
		checkadditionalconstraint(root,id,modeltype)
		{
			let rel_lists = ['rel_general_root','rel_general_general','rel_leaf_root','rel_leaf_general'];
			for(let x = 0; x < rel_lists.length; x++)
			{
				if(root[rel_lists[x]] !== undefined)
				{
					if(Array.isArray(root[rel_lists[x]]))
					{
						for(let i = 0; i < root[rel_lists[x]].length; i++)
						{
							...
							root[rel_lists[x]][i].mxCell['@source'] === id)
							{
								if(root[rel_lists[x]][i]['@relType'] === 'requires' || root[rel_lists[x]][i]['@relType'] === 'excludes')
									return true;
							}
						}
					}
					else if(root[rel_lists[x]].mxCell['@parent'] === modeltype && 
							root[rel_lists[x]].mxCell['@source'] === id)
					{
						if(root[rel_lists[x]]['@relType'] === 'requires' || root[rel_lists[x]]['@relType'] === 'excludes')
							return true;
					}
				}
			}
