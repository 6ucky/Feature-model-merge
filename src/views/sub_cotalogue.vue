	...
	<div>
		<div>
        	<div>
        		...
        		<input type="text" v-model="user_name" class="" name="name"/>
    		</div>
			<button @click="updateuserlist()">Add user</button>
        	<button @click="mergelist()">Merge</button>
			<button @click="inituserlist()">Clear</button>
      	</div>
		<div class="col-sm-3">
			<select v-model="selected_user" transfer="true" @change="updateselecttree()">
        		<option v-for="item in user_list" :value="item" :key="item">{{ item }}</Option>
    		</select>
		</div>
	...
	methods: {
		mergelist(){
			...
			if(this.user_list.length !== 0)
			{
				...
				{
					if(localStorage.getItem('User+' + this.user_list[i]))
					{
						let selection_list = [];
						selection_list = JSON.parse(localStorage.getItem('User+' + this.user_list[i]));
						temp = temp.concat(selection_list);
					}
					
				}
				for(let i = 0; i < this.data.length; i++)
				{
					if(temp.includes(this.data[i].data.nodeId))
						this.data[i].data.tick = true;
				}
				this.user_list = [];
				this.user_list.push('Merge');
				this.selected_user = 'Merge';
				localStorage.clear();  
				localStorage.setItem('User+Merge', JSON.stringify(temp));
			}
		},
		updateselecttree(){
			for(let i = 0; i < this.data.length; i++)
			...
			if(localStorage.getItem('User+' + this.selected_user))
			{
				let selection_list = [];
				selection_list = JSON.parse(localStorage.getItem('User+' + this.selected_user));
				for(let i = 0; i < this.data.length; i++)
				{
					if(selection_list.includes(this.data[i].data.nodeId))
						this.data[i].data.tick = true;
				}
			}
		},
		updateuserlist(){
      		if(this.user_name === '')
				...
				this.user_list.push(this.user_name);
      		else
      		{
        		for(let i = 0; i < this.user_list.length; i++)
        		{
          			if(this.user_name !== this.user_list[i])
            			this.user_list.push(this.user_name);
        		}
			}
    	},
		...
		itemclick(index){
			...
			localStorage.setItem('User+' + this.selected_user, JSON.stringify(selected_list));
		},
		/**
